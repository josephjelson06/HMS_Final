...
import { X, Check, ChevronDown, Layout, Monitor, Headphones, Save, Sparkles } from 'lucide-react';
...
const UpdatePlanPanel: React.FC<UpdatePlanPanelProps> = ({ isOpen, onClose, plan, onUpdate }) => {
  ...
  const [availableModules] = useState([
    ...
    "Advanced Reports", "Inventory Tracking", "Staff Management", 
    "Multi-property Support", "API Access", "Custom Branding"
  ]);
  ...
  useEffect(() => {
    if (isOpen && plan) {
      ...
      setMaxRooms(plan.rooms);
      setMaxKiosks(plan.kiosks);
      setSupportLevel(plan.support);
      setSelectedModules(plan.included || []);
    }
  }, [isOpen, plan]);
      ...
      >
        <div className="h-full flex flex-col relative overflow-hidden">
            <div className={`px-8 py-6 border-b shrink-0 flex items-center justify-between ${isDarkMode ? 'border-white/10' : 'border-gray-200/50'}`}>
                <div>
                    <h2 className="text-2xl font-black dark:text-white tracking-tighter uppercase">Update Catalog Offering</h2>
                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-1">Editing Plan ID: {plan?.id}</p>
                </div>
                <button onClick={onClose} className="p-3 rounded-full hover:bg-white/10 transition-colors text-gray-400">
                    <X size={24} />
                </button>
            </div>
                ...
                <section>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div className="col-span-2">
                            ...
                            <input type="text" value={planName} onChange={(e) => setPlanName(e.target.value)} className={inputClass} />
                        </div>
                        <div>
                            <label className={labelClass}>Monthly Price (INR) *</label>
                            <div className="relative">
                                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">Ôé╣</span>
                                <input type="number" value={monthlyPrice} onChange={(e) => setMonthlyPrice(Number(e.target.value))} className={`${inputClass} pl-8`} />
                            </div>
                        </div>
                    </div>
                </section>
                ...
                <section>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" value={maxRooms} onChange={(e) => setMaxRooms(Number(e.target.value))} className={inputClass} />
                        </div>
                        <div>
                            <label className={labelClass}>Max Kiosk Units *</label>
                            <input type="number" value={maxKiosks} onChange={(e) => setMaxKiosks(Number(e.target.value))} className={inputClass} />
                        </div>
                    </div>
                </section>
                ...
                <section>
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-4">
                        {availableModules.map((module) => (
                            ...
                            >
                                {module}
                                {selectedModules.includes(module) && <Check size={12} strokeWidth={4} />}
                            </button>
                        ))}
                    </div>
                </section>

                {/* Logistics & Notes - Screenshot 1 Style */}
                <section>
                    <div className={screenshotHeaderClass}>
                    ...
                    <div className="space-y-8">
                        <div>
                            ...
                            <GlassDropdown 
                                ...
                                className="w-full"
                                trigger={
                                    <div className={`${inputClass} flex items-center justify-between cursor-pointer py-4 px-6 bg-white dark:bg-black/20`}>
                                        <span className="text-slate-900 dark:text-white">{supportLevel}</span>
                                        <ChevronDown size={18} className="text-slate-400" />
                                    </div>
                                }
                                items={[
                                    { label: 'Email Only', onClick: () => setSupportLevel('Email Only'), variant: supportLevel === 'Email Only' ? 'selected' : 'default' },
                                    { label: 'Standard (Phone + Email)', onClick: () => setSupportLevel('Standard (Phone + Email)'), variant: supportLevel === 'Standard (Phone + Email)' ? 'selected' : 'default' },
                                    { label: 'Priority 24/7', onClick: () => setSupportLevel('Priority 24/7'), variant: supportLevel === 'Priority 24/7' ? 'selected' : 'default' },
                                    { label: 'Dedicated Account Manager', onClick: () => setSupportLevel('Dedicated Account Manager'), variant: supportLevel === 'Dedicated Account Manager' ? 'selected' : 'default' }
                                ]}
                            />
                        </div>
                    </div>
            ...
            <div className={`px-8 py-6 border-t shrink-0 flex justify-end gap-3 ${isDarkMode ? 'border-white/10 bg-black/20' : 'border-gray-200/50 bg-white/50'}`}>
                ...
                <button 
                  onClick={handleSave}
                  className="px-10 py-3 rounded-2xl text-[10px] font-bold uppercase tracking-widest bg-gray-900 dark:bg-white text-white dark:text-black shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center gap-2"
                >
