...
import { X, Binary, Cpu, ChevronRight, ChevronLeft, CheckCircle2, ShieldCheck, Upload, Zap, Terminal, Globe } from 'lucide-react';
    ...
    <div className="flex items-center gap-2 mb-10">
      {[1, 2, 3].map((s) => (
        <React.Fragment key={s}>
          <div className={`w-10 h-10 rounded-2xl flex items-center justify-center text-xs font-black transition-all duration-500 ${s === step ? 'bg-blue-600 text-white shadow-xl scale-110' : s < step ? 'bg-emerald-500 text-white' : 'bg-black/5 dark:bg-white/5 text-gray-500'}`}>
          ...
          <GlassCard noPadding className="shadow-[0_50px_100px_rgba(0,0,0,0.6)] border-white/10 overflow-hidden">
            ...
            <div className="px-10 py-8 border-b border-white/5 flex justify-between items-center bg-black/5">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-2xl bg-blue-600 text-white flex items-center justify-center shadow-2xl shadow-blue-900/40">
                  <Binary size={28} />
                </div>
                <div>
                  <h2 className="text-2xl font-black dark:text-white tracking-tighter uppercase leading-none">New Firmware Release</h2>
                  <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mt-1.5">Production Build Forge</p>
                </div>
              </div>
              ...
              <div className="flex-1">
                {step === 1 && (
                  <div className="space-y-8 animate-in fade-in slide-in-from-right-8 duration-500">
                    <div className="flex items-center gap-3 text-blue-500">
                        ...
                        <h3 className="text-sm font-black uppercase tracking-[0.2em]">Build Metadata</h3>
                    </div>
                    <div className="grid grid-cols-2 gap-6">
                        ...
                        <div className="col-span-2">
                           ...
                           <div className="p-10 rounded-[2.5rem] border-2 border-dashed border-white/10 bg-black/5 dark:bg-white/[0.01] hover:border-blue-500/50 transition-all flex flex-col items-center justify-center gap-4 cursor-pointer group">
                              <Upload size={32} className="text-gray-500 group-hover:scale-110 transition-transform" />
                              <span className="text-xs font-black uppercase text-gray-500 tracking-widest">Click to upload build artifact</span>
                              <span className="text-[9px] font-bold text-gray-600 italic">Max size: 128 MB</span>
                           </div>
                        </div>
                    </div>
                  </div>
                ...
                {step === 2 && (
                  <div className="space-y-8 animate-in fade-in slide-in-from-right-8 duration-500">
                    <div className="flex items-center gap-3 text-emerald-500">
                        ...
                        <h3 className="text-sm font-black uppercase tracking-[0.2em]">Compatibility Mapping</h3>
                    </div>
                           ...
                           <div className="grid grid-cols-2 gap-3">
                              {['Gen 3 Floor Unit', 'Gen 3 Desktop', 'Legacy Hardware', 'Alpha Prototypes'].map(tier => (
                                <label key={tier} className="flex items-center justify-between p-4 rounded-2xl border border-white/5 bg-black/5 dark:bg-white/[0.02] cursor-pointer hover:bg-white/5 transition-all">
                                   <span className="text-[11px] font-bold dark:text-gray-300 uppercase">{tier}</span>
                                   <input type="checkbox" className="w-4 h-4 rounded-md border-white/10 bg-black text-blue-600" defaultChecked />
                                </label>
                              ))}
                           </div>
                        </div>
                ...
                {step === 3 && (
                  <div className="flex flex-col items-center justify-center text-center space-y-8 animate-in zoom-in-95 duration-500">
                     <div className="w-24 h-24 rounded-[2.5rem] bg-blue-600 text-white flex items-center justify-center shadow-2xl shadow-blue-900/40 relative">
                     ...
                     <div className="w-full grid grid-cols-2 gap-4 p-8 rounded-[3rem] bg-black/5 dark:bg-white/[0.02] border border-white/5">
                        <div className="text-left">
                           <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Build ID</p>
                           <p className="text-sm font-black dark:text-white uppercase font-mono">HASH: 4X9F2A</p>
                        </div>
                        <div className="text-right">
                           <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Global Availability</p>
                           <p className="text-sm font-black text-blue-500 uppercase flex items-center justify-end gap-2">
              ...
              <div className="mt-12 flex justify-between items-center border-t border-white/5 pt-8">
                ...
                <button 
                  ...
                  className={`flex items-center gap-3 px-12 py-4 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest shadow-2xl hover:scale-105 active:scale-95 transition-all
                    ${step === totalSteps ? 'bg-blue-600 text-white shadow-blue-900/40' : 'bg-gray-900 dark:bg-white text-white dark:text-black shadow-blue-900/20'}
                  `}
                >
