    ...
    >
      <div className="flex h-[80vh]">
        ...
        <div className="flex-1 flex flex-col min-w-0">
           ...
           <div className="px-8 py-6 border-b border-white/10 flex justify-between items-center bg-black/5">
              <div className="flex items-center gap-6">
                 <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white shadow-xl ${
                    ticket.priority === 'Critical' ? 'bg-red-600' : 'bg-accent-strong'
                 }`}>
                    <LifeBuoy size={32} />
                 </div>
                 <div>
           ...
           <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
              <div className="p-6 rounded-[2rem] bg-black/5 dark:bg-white/[0.02] border border-white/5 mb-8">
                 <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">Original Issue Detail</p>
                 ...
                 {messages.map((m, i) => (
                    <div key={i} className={`flex flex-col ${m.isMe ? 'items-end' : 'items-start'}`}>
                       ...
                       <div className={`max-w-[85%] p-4 rounded-3xl text-sm font-medium leading-relaxed ${
                          m.isMe ? 'bg-accent-strong text-white rounded-tr-none' : 'bg-black/5 dark:bg-white/5 dark:text-gray-200 border border-white/5 rounded-tl-none'
                       }`}>
                          {m.text}
                       </div>
                    </div>
        ...
        <div className="w-80 bg-black/10 dark:bg-white/[0.02] border-l border-white/10 overflow-y-auto p-8 space-y-10 custom-scrollbar">
           ...
           {/* Ticket Metadata */}
           <section>
              <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500 mb-6">Support Intelligence</h3>
              <div className="space-y-3">
                 <div className="p-4 rounded-2xl bg-black/20 border border-white/5">
                    <p className="text-[9px] font-bold text-gray-500 uppercase mb-1">Issue Category</p>
                    <div className="flex items-center gap-2">
                       <Monitor size={14} className="text-accent" />
                       <span className="text-xs font-bold dark:text-white">{ticket.category}</span>
                    </div>
                 </div>
                 <div className="p-4 rounded-2xl bg-black/20 border border-white/5">
                    <p className="text-[9px] font-bold text-gray-500 uppercase mb-1">Assessed Priority</p>
                    <span className="text-xs font-bold text-accent">{ticket.priority}</span>
                 </div>
              </div>
           </section>
           ...
           {/* SLA Progress */}
           <section>
              <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500 mb-6">Service Level Status</h3>
              <div className="p-6 rounded-[2rem] bg-black/5 dark:bg-white/5 border border-white/5">
                 <div className="flex justify-between items-center mb-4">
                    <span className="text-[9px] font-medium text-gray-500 uppercase">Target Resolution</span>
                    <span className="text-[10px] font-bold dark:text-white">4 Hours</span>
                 </div>
           ...
           {/* ATC Ownership */}
           <section>
              <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500 mb-6">ATC Lead Agent</h3>
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-black/5 dark:bg-white/5 border border-white/5">
                 <div className="w-10 h-10 rounded-full bg-accent/10 flex items-center justify-center text-accent-strong font-bold text-xs">VP</div>
                 <div>
                    <h4 className="text-xs font-bold dark:text-white">Vikram Patel</h4>
                    <p className="text-[9px] font-medium text-gray-500 uppercase tracking-tighter">Support Engineer</p>
                 </div>
              </div>
           </section>
