...
export class MockHotelStaffRepository implements IHotelStaffRepository {
  async getAllStaff(): Promise<HotelStaffMember[]> {
    return mockStaff;
  }

  async getAllRoles(): Promise<HotelRole[]> {
    return rolesData;
  }

  async getStaffById(id: string): Promise<HotelStaffMember | null> {
    return mockStaff.find((s) => s.id === id) ?? null;
  }

  async createStaff(data: Omit<HotelStaffMember, 'id'>): Promise<HotelStaffMember> {
    return { id: `S-${Date.now()}`, ...data };
  }

  async updateStaff(id: string, data: Partial<HotelStaffMember>): Promise<HotelStaffMember> {
