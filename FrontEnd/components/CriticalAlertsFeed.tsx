...
import { AlertCircle, AlertTriangle, Clock, ChevronRight, Zap } from 'lucide-react';
...
const AlertItem = ({ type, message, time, onAction }: any) => {
  ...
  return (
    <div className="flex items-center gap-4 p-5 hover:bg-black/5 dark:hover:bg-white/5 transition-all group border-b border-white/5 last:border-b-0">
      <div className={`p-2.5 rounded-xl shrink-0 ${isCritical ? 'bg-red-500/10 text-red-500' : 'bg-amber-500/10 text-amber-500'}`}>
...
const CriticalAlertsFeed: React.FC = () => {
  return (
    <GlassCard noPadding clipContent className="h-full flex flex-col shadow-2xl border-white/10">
      ...
      <div className="px-8 py-6 border-b border-gray-100 dark:border-white/5 flex items-center justify-between bg-black/5 dark:bg-white/[0.02]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-black text-slate-900 dark:text-white uppercase tracking-tighter">Critical Alerts</h3>
