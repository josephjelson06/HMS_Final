...
const Reports: React.FC = () => {
  ...
  const reportConfigs: Record<string, { title: string, data: any[], cols: Column[] }> = {
     'hotels': {
        ...
        data: hotels,
        cols: [
            { key: 'name', label: 'Hotel Name', render: (row) => <span className="font-bold">{row.name}</span> },
            ...
            { key: 'plan', label: 'Plan' },
            { key: 'gstin', label: 'GSTIN', render: (row) => <span className="font-mono text-xs">{row.gstin}</span> },
            ...
            { key: 'creationDate', label: 'Onboarded', render: (row) => new Date(row.creationDate || Date.now()).toLocaleDateString() }
        ]
     },
     'subscriptions': {
        ...
        data: subscriptions,
        cols: [
            { key: 'id', label: 'Sub ID', render: (row) => <span className="font-mono text-xs">#{row.id}</span> },
            { key: 'hotelName', label: 'Hotel Name', render: (row) => <span className="font-bold">{row.hotelName || `Hotel #${row.hotelId}`}</span> },
            ...
            { key: 'nextBillingDate', label: 'Renewal', render: (row) => new Date(row.nextBillingDate).toLocaleDateString() }
        ]
     },
     'invoices': {
        ...
        data: invoices,
        cols: [
            { key: 'id', label: 'Invoice #', render: (row) => <span className="font-mono text-xs">{row.id}</span> },
            { key: 'date', label: 'Date Issued', render: (row) => new Date(row.date).toLocaleDateString() },
            { key: 'hotelName', label: 'Billed To', render: (row) => <span className="font-bold">{row.hotelName || 'Unknown Hotel'}</span> },
            { key: 'amount', label: 'Total', render: (row) => `₹${row.total || row.amount}` },
            { key: 'status', label: 'Payment', render: (row) => (
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide ${
