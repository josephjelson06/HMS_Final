...
const ConfirmationModal: React.FC<ConfirmationModalProps> = ({
  ...
  const iconBgClass = isDanger
    ...
    : isWarning
      ? "bg-amber-100 dark:bg-amber-900/30 shadow-amber-900/20"
      : "bg-accent/10 shadow-accent/20";
    ...
    <ModalShell
      ...
      headerContent={
        <div className="flex items-center gap-4">
          <div
            className={`w-12 h-12 rounded-2xl ${iconBgClass} ${iconColorClass} flex items-center justify-center shadow-lg`}
          >
            {Icon && <Icon size={24} />}
          </div>
          <div>
            <h2 className="text-lg font-black dark:text-white tracking-tight uppercase">
              {title}
            </h2>
          </div>
        </div>
      ...
      footer={
        <div className="flex justify-end gap-3 w-full">
          ...
          <Button
            ...
            } // Manual override for warning color if needed
          >
            {confirmLabel}
          </Button>
        </div>
