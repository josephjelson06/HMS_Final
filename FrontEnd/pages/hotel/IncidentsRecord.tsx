...
const IncidentsRecord: React.FC = () => {
  ...
  const PriorityBadge = ({ level }: { level: IncidentPriority }) => {
    ...
    return <span className={`inline-block px-3 py-1.5 rounded text-[8px] font-black uppercase tracking-widest leading-none whitespace-nowrap ${styles[level]}`}>{level}</span>;
  };
        ...
        <div className="space-y-4">
          {columnIncidents.map(inc => (
            ...
            >
              ...
              <h4 className="text-sm font-black dark:text-white leading-snug mb-3 group-hover:text-blue-600 dark:group-hover:text-orange-500 transition-colors">
                {inc.subject}
              </h4>
  ...
  return (
    <div className="p-8 space-y-8 min-h-screen pb-24 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-gray-900 dark:text-white tracking-tighter uppercase">Incident Records</h1>
          <p className="text-sm font-bold text-gray-500 uppercase tracking-widest mt-1">Operational Issue & Problem Tracker</p>
        </div>
        <div className="flex gap-3">
          <div className="flex bg-black/5 dark:bg-white/5 rounded-2xl p-1.5 border border-white/5">
            <button 
              onClick={() => setViewMode('table')} 
              className={`p-2 rounded-xl transition-all ${viewMode === 'table' ? 'bg-white dark:bg-white/10 shadow-lg text-blue-600 dark:text-orange-500' : 'text-gray-400 hover:text-white'}`}
            >
              <List size={20} />
            </button>
            <button 
              onClick={() => setViewMode('kanban')} 
              className={`p-2 rounded-xl transition-all ${viewMode === 'kanban' ? 'bg-white dark:bg-white/10 shadow-lg text-blue-600 dark:text-orange-500' : 'text-gray-400 hover:text-white'}`}
            >
              <LayoutGrid size={20} />
            </button>
          </div>
          <button 
            ...
            className="flex items-center gap-2 bg-red-600 text-white px-8 py-3.5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-2xl hover:scale-[1.02] active:scale-95 transition-all"
          >
            <Flag size={18} strokeWidth={3} /> Raise New Incident
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
         ...
         <SummaryCard label="Maintenance" value="08" sub="On-going repair" icon={Wrench} color="text-orange-500" />
         <SummaryCard label="Housekeeping" value="14" sub="Priority Cleaning" icon={Brush} color="text-blue-500" />
        ...
        <div className="flex p-1.5 rounded-2xl bg-black/5 dark:bg-white/5 border border-white/5 w-fit overflow-x-auto no-scrollbar">
          {['All', 'Open', 'In Progress', 'Resolved', 'Closed'].map((tab) => (
            <button 
              ...
              className={`px-8 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === tab ? 'bg-white dark:bg-white/10 text-gray-900 dark:text-white shadow-lg' : 'text-gray-500 hover:text-gray-900 dark:hover:text-gray-300'}`}
            >
              {tab}
            </button>
          ))}
        ...
        <div className="flex-1 w-full lg:max-w-md">
            <div className="relative group">
                ...
                <input
                  ...
                  className="block w-full pl-16 pr-6 py-4 border border-white/10 rounded-[1.5rem] bg-white/40 dark:bg-black/40 text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-blue-500/10 dark:focus:ring-orange-500/10 sm:text-sm shadow-sm"
          ...
          <GlassCard noPadding className="overflow-hidden border-white/10 shadow-2xl">
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-black/10 dark:bg-white/5 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 border-b border-white/5">
                    ...
                    >
                      ...
                      <td className="px-8 py-6">
                        <div className="flex flex-col">
                           <span className="text-sm font-black dark:text-white mb-1 group-hover:text-orange-500 transition-colors uppercase tracking-tight">{inc.subject}</span>
                           <span className="text-[10px] font-bold text-gray-500 uppercase flex items-center gap-1"><DoorOpen size={10} /> Room #{inc.room}</span>
                        </div>
                      </td>
                      <td className="px-8 py-6"><span className="inline-block px-2 py-1 text-[10px] font-black uppercase text-gray-400 bg-black/5 dark:bg-white/5 border border-white/5 rounded leading-none whitespace-nowrap">{inc.category}</span></td>
