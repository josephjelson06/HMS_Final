  ...
  return (
    <div className="p-4 md:p-8 space-y-6 min-h-screen pb-20 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col lg:flex-row justify-between lg:items-center gap-6 p-8 relative z-10">
                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-gray-600 dark:text-gray-400">
                        <a 
                          ...
                          className="flex items-center gap-1.5 hover:text-blue-600 dark:hover:text-orange-500 transition-colors"
                        >
                          <MapPin size={14} />
                        ...
                        <div className="flex items-center gap-1.5">
                          <Mail size={14} />
                          <a href="mailto:ops@royalorchid.com" className="hover:text-blue-600 dark:hover:text-orange-500 transition-colors">ops@royalorchid.com</a>
                        </div>
                    </div>
                </div>
            </div>
            <div className="flex items-center gap-3">
                ...
                <button 
                  ...
                  className="px-5 py-2.5 rounded-xl text-sm font-bold shadow-lg shadow-orange-500/20 bg-gray-900 dark:bg-orange-500 text-white hover:scale-105 transition-all flex items-center gap-2"
                >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {[
          { label: 'Subscription', value: 'Enterprise', color: 'text-orange-500', footer: 'Renewing in 12 days' },
          { label: 'Status', value: 'Active', sub: 'Since June 2024', color: 'text-emerald-500', footer: 'Account in good standing' },
          { label: 'Total Revenue', value: '₹59.0k', color: 'text-blue-500', footer: 'Life-to-date yielding' },
        ].map((m, i) => (
          <GlassCard key={i} noPadding clipContent className="flex flex-col h-32 border-white/5 dark:border-white/10">
            ...
            <div className="bg-black/5 dark:bg-white/[0.03] px-6 py-2 border-t border-white/5">
               <p className="text-[8px] font-black uppercase text-gray-500 tracking-wider">{m.footer}</p>
            </div>
          </GlassCard>
        ))}
      </div>
        ...
        {activeTab === 'Overview' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <GlassCard noPadding clipContent className="border-white/5 dark:border-white/10 flex flex-col">
              ...
              <button className="w-full py-4 bg-black/5 dark:bg-white/5 text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 dark:hover:bg-orange-500 hover:text-white transition-all border-t border-white/5">Change Subscription Plan</button>
            </GlassCard>
          </div>
        )}
        ...
        {activeTab === 'Invoice History' && (
          <GlassCard noPadding clipContent className="overflow-hidden border-white/5 dark:border-white/10 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <table className="w-full text-left">
              <thead className="bg-black/5 dark:bg-white/5 text-[10px] font-black uppercase tracking-widest text-gray-500">
              ...
              <tbody className="divide-y divide-white/5">
                {[
                  ...
                  <tr key={i} className="hover:bg-white/5 transition-colors group cursor-pointer">
                    <td className="px-8 py-6 text-sm font-mono font-bold text-blue-600 dark:text-orange-500">{inv.id}</td>
                    <td className="px-8 py-6 text-[10px] font-black uppercase tracking-widest dark:text-gray-400">{inv.p}</td>
                    <td className="px-8 py-6 text-sm font-black text-right dark:text-white tracking-tighter">₹{inv.a.toLocaleString()}</td>
                    <td className="px-8 py-6 text-center">
                      <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border ${inv.s === 'paid' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-red-500/10 text-red-500 border-red-500/20'}`}>
                        {inv.s}
                      </span>
                    </td>
